...
function OverviewSection({ leads, tasks, users, user, onTaskUpdate }) {
  ...
  const getFilteredTasks = () => {
    ...
    return tasks.filter(task => {
      ...
      if (dateFilter === 'today') return taskDate.getTime() === now.getTime();
      if (dateFilter === 'week') {
  ...
  return (
    <div className="p-6">
      ...
      {/* Calendar */}
      <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 mb-6">
      ...
      {/* Today's Appointments & To-Dos */}
      <div className="grid grid-cols-2 gap-6">
        {/* Today's Appointments */}
        <div className="bg-slate-800/50 rounded-xl border border-slate-700/50">
          <div className="p-4 border-b border-slate-700/50 flex items-center justify-between">
            <h2 className="text-lg font-medium text-white">Today's Appointments</h2>
            <span className="text-sm text-slate-400">{today.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}</span>
          </div>
          <div className="p-4">
            {todaysTasks.length === 0 ? (
              <div className="text-center py-6 text-slate-400 text-sm">No appointments today</div>
            ) : (
                ...
                {todaysTasks.map(task => (
                  <div key={task.id} className="flex items-center gap-3 p-3 bg-slate-900/50 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      {task.description && <div className="text-slate-400 text-xs">{task.description}</div>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="bg-slate-800/50 rounded-xl border border-slate-700/50">
          <div className="p-4 border-b border-slate-700/50 flex items-center justify-between">
            ...
            <div className="flex gap-1">
              <button onClick={() => setDateFilter('today')} className={`px-3 py-1 text-xs rounded-lg transition ${dateFilter === 'today' ? 'bg-rust text-white' : 'text-slate-400 hover:text-white'}`}>Today</button>
              <button onClick={() => setDateFilter('week')} className={`px-3 py-1 text-xs rounded-lg transition ${dateFilter === 'week' ? 'bg-rust text-white' : 'text-slate-400 hover:text-white'}`}>Week</button>
              <button onClick={() => setDateFilter('month')} className={`px-3 py-1 text-xs rounded-lg transition ${dateFilter === 'month' ? 'bg-rust text-white' : 'text-slate-400 hover:text-white'}`}>Month</button>
            </div>
          </div>
          <div className="p-4">
